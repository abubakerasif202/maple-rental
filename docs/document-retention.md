...
## External scheduler preparation (disabled)
...
Vercel does not run a persistent cron process for this repository. If approved, configure an explicitly owned external scheduler to invoke `npm run clean:documents` with the same private Supabase server credentials. This command is dry-run only, emits one structured JSON summary, and exits non-zero when storage, hold lookup, or audit persistence fails. Do not schedule `clean:documents:apply`; apply mode additionally requires `DOCUMENT_CLEANUP_APPLY_ENABLED=true`, which must remain unset in normal service and scheduler configuration.
